...
# Denominator: only rentals where delay is known (excludes NaN)
n_with_delay_info = df_filtered["delay_at_checkout_in_minutes"].notna().sum()
pct_late = (total_late / n_with_delay_info * 100) if n_with_delay_info > 0 else 0.0

# FIX: guard against division by zero when df_filtered is empty
if total_rentals > 0:
    blocked = df_filtered[
...
with col_g2:
    ...
    # Always computed on full df (not df_filtered) to keep both bars visible
    compare = (
    ...
    fig2.update_layout(
        showlegend=False,
        height=350,
        yaxis=dict(range=[0, 70], title="% late"),
    )
...
# FIX: use .loc with nearest match instead of strict equality → avoids IndexError
# if threshold value somehow doesn't exist exactly in the simulated range
...
if st.button("🔮 Predict price"):
    # Build feature vector — order must match feature_names.json in the API
    features = [
        mileage,
        engine_power,
